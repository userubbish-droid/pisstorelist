...
PHP;
        ...
        if ($written === false) {
            $error = '写入 config.php 失败：请确认文件权限允许写入（Hostinger File Manager 可改权限）';
        } else {
            // 重新加载配置
            require __DIR__ . '/config.php';
            init_db();
            ensure_admin_exists(ADMIN_USERNAME, ADMIN_PASSWORD);
            $ok = '安装完成：管理员已创建，数据库已初始化。请删除 install.php 以确保安全。';
        }
    }
}
      ...
      <div class="bg-white border rounded-xl p-6">
        ...
        <?php if ($ok): ?>
          <div class="mt-4 p-3 rounded bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm">
            <?= h($ok) ?>
            <div class="mt-2">
              <a class="underline" href="/index.php?page=login">去登录</a>
            </div>
          </div>
        <?php endif; ?>
        ...
        <div class="mt-4 text-xs text-slate-500">
          安装完成后请删除 <code class="px-1 bg-slate-100 rounded">install.php</code>。
        </div>
      </div>
    </main>
  </body>
